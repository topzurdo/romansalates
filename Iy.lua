...
IY_REPO_RAW = "https://raw.githubusercontent.com/topzurdo/romansalates/main/"
IY_DECOMPILER_URL = "http://127.0.0.1:31337/decompile"

local function iyFetchRepo(fileName)
	if readfile and isfile then
		for _, path in ipairs({fileName, "infiniteyield/" .. fileName, "bytecodeveryop/" .. fileName}) do
			if isfile(path) then
				return readfile(path)
			end
		end
	end
	return game:HttpGet(IY_REPO_RAW .. fileName)
end

local function iyBase64Encode(data)
	...
	local out, pad = {}, ''
	for i = 1, #data, 3 do
		local a, c, d = string.byte(data, i), string.byte(data, i + 1) or 0, string.byte(data, i + 2) or 0
		local n = a * 65536 + c * 256 + d
		local i1 = math.floor(n / 262144) % 64 + 1
		local i2 = math.floor(n / 4096) % 64 + 1
		local i3 = math.floor(n / 64) % 64 + 1
		local i4 = n % 64 + 1
		table.insert(out, string.sub(b, i1, i1))
		table.insert(out, string.sub(b, i2, i2))
		table.insert(out, string.sub(b, i3, i3))
		table.insert(out, string.sub(b, i4, i4))
	end
	local rem = #data % 3
	if rem == 2 then
		out[#out] = '='
...
local function iyDecompileBytecode(bytecode)
	...
	local ok, res = pcall(function()
		return httprequest({
			Url = IY_DECOMPILER_URL,
			Method = 'POST',
			Headers = {['Content-Type'] = 'application/json'},
			Body = HttpService:JSONEncode({bytecode = iyBase64Encode(bytecode), mode = 'decompile'})
		})
	end)
	if not ok then
...
local function iyGetBytecodeTarget(args)
	local getBytecode = getscriptbytecode or getsbc or nil
	if not getBytecode then
		return nil, 'getscriptbytecode is not available in this executor'
	end
	local target = nil
	if args[1] and args[1] ~= '' then
		target = getInstance(args[1])
	else
		target = IYMouse.Target
	end
	if not target then
		return nil, 'no target instance'
	end
	if target:IsA('LocalScript') or target:IsA('ModuleScript') or target:IsA('Script') then
		return target, nil
	end
	local script = target:FindFirstAncestorWhichIsA('LuaSourceContainer')
	if script then
		return script, nil
	end
	return nil, 'target is not a script'
end

function iyDecompileScript(script)
	local getBytecode = getscriptbytecode or getsbc
	if not getBytecode then
		error('getscriptbytecode is not available in this executor')
	end
	...
	local code, err = iyDecompileBytecode(bytecode)
	if not code then
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('" .. IY_REPO_RAW .. "Iy.lua'))()")
	end
end)
...
addcmd('decompile', {'luaudec', 'bytecodedec'}, function(args, speaker)
	local getBytecode = getscriptbytecode or getsbc
	if not getBytecode then
		notify('Decompile', 'getscriptbytecode is not available in this executor')
		return
	end
	local script, err = iyGetBytecodeTarget(args)
	if not script then
		notify('Decompile', err or 'could not resolve script')
		return
	end
	local ok, bytecode = pcall(getBytecode, script)
	if not ok or type(bytecode) ~= 'string' or bytecode == '' then
		notify('Decompile', 'failed to read bytecode')
		return
	end
	local code, derr = iyDecompileBytecode(bytecode)
	if not code then
		notify('Decompile', derr or 'decompile failed')
		return
	end
	local outName = (script.Name or 'script') .. '.decompiled.lua'
	if makefolder and isfolder and writefile then
		pcall(function()
			if not isfolder('infiniteyield') then makefolder('infiniteyield') end
			if not isfolder('infiniteyield/decompiled') then makefolder('infiniteyield/decompiled') end
			writefile('infiniteyield/decompiled/' .. outName, code)
		end)
	end
	if everyClipboard then
		everyClipboard(code)
	end
	notify('Decompile', 'Saved to infiniteyield/decompiled/' .. outName .. (everyClipboard and ' (copied)' or ''))
end)
...
addcmd("explorer", {"dex"}, function(args, speaker)
	...
	local ok, err = pcall(function()
		loadstring(iyFetchRepo("dex.lua"))()
	end)
	if ok then
		notify("DEX", "Loaded. Keep bytecode_decompiler.exe running for View Script")
	else
		notify("DEX", "Failed to load: " .. tostring(err))
	end
end)
