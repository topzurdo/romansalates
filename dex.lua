...
Main = (function()
	...
	Main.InitEnv = function()
		...
		local function iyDexBase64(data)
			...
			for i = 1, #data, 3 do
				...
				table.insert(out, string.sub(b, math.floor(n / 262144) % 64 + 1, math.floor(n / 262144) % 64 + 1))
				table.insert(out, string.sub(b, math.floor(n / 4096) % 64 + 1, math.floor(n / 4096) % 64 + 1))
				table.insert(out, string.sub(b, math.floor(n / 64) % 64 + 1, math.floor(n / 64) % 64 + 1))
				table.insert(out, string.sub(b, n % 64 + 1, n % 64 + 1))
			end
			local rem = #data % 3
			if rem == 2 then out[#out] = "=" elseif rem == 1 then out[#out], out[#out - 1] = "=", "=" end
			return table.concat(out)
		end

		local function iyBytecodeDecompile(script)
		...
		env.decompile = iyBytecodeDecompile
