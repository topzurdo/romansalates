...
local function simpleSpyBase64Encode(data)
	...
	for i = 1, #data, 3 do
		...
		table.insert(out, string.sub(alphabet, math.floor(n / 262144) % 64 + 1, math.floor(n / 262144) % 64 + 1))
		table.insert(out, string.sub(alphabet, math.floor(n / 4096) % 64 + 1, math.floor(n / 4096) % 64 + 1))
		table.insert(out, string.sub(alphabet, math.floor(n / 64) % 64 + 1, math.floor(n / 64) % 64 + 1))
		table.insert(out, string.sub(alphabet, n % 64 + 1, n % 64 + 1))
	end
	local rem = #data % 3
	if rem == 2 then
		out[#out] = "="
...
local function simpleSpyDecompile(script)
	if typeof(script) ~= "Instance" or not script:IsA("LuaSourceContainer") then
		error("expected LocalScript, ModuleScript, or Script")
	end
	if getgenv().IY_decompile then
		local ok, res = pcall(getgenv().IY_decompile, script)
	...
	local httpReq = httprequest or request or (syn and syn.request)
	if not getBytecode or not httpReq then
		error("[executor_missing] need getscriptbytecode and httprequest (run bytecode_decompiler.exe --serve)")
	end
	...
	if not bcOk or type(bytecode) ~= "string" or bytecode == "" then
		...
		error("[bytecode_unavailable] getscriptbytecode failed (" .. hint .. ") for " .. script:GetFullName())
	end
	if #bytecode < 8 then
		error("[bytecode_unavailable] bytecode too small (" .. #bytecode .. " bytes)")
	end
	...
	local res = httpReq({
		...
		Body = http:JSONEncode({
			bytecode = simpleSpyBase64Encode(bytecode),
			mode = "decompile",
		}),
	})
	if type(res) == "table" and (res.StatusCode == 200 or res.Success) and res.Body then
		...
		if body.ok and type(body.code) == "string" then
			...
			if type(body.warnings) == "table" and #body.warnings > 0 then
				...
				if code ~= "" and not code:find("^%-%- warning:", 1) then
	...
	error("[server_unreachable] is bytecode_decompiler.exe running? (--serve)")
end
...
--- Attempts to decompile the source script via bytecode_decompiler.exe
newButton("Decompile",
    function()
        return "Decompile source script via bytecode_decompiler.exe (--serve on PC)"
    end,
    function()
        if selected and selected.Source then
            local Source = selected.Source
            if typeof(Source) ~= "Instance" or not Source:IsA("LuaSourceContainer") then
                TextLabel.Text = "Source not found!"
                return
            end
            if not DecompiledScripts[Source] then
                codebox:setRaw("--[[Decompiling via bytecode_decompiler...]]")
                local ok, result = xpcall(function()
                    local decompiledsource = simpleSpyDecompile(Source)
                    ...
                    DecompiledScripts[Source] = decompiledsource
                end, function(err)
                    return tostring(err)
                end)
                if not ok then
                    codebox:setRaw("--[[\nDecompile failed:\n" .. tostring(result) .. "\n\nRun bytecode_decompiler.exe --serve on your PC\n]]")
                    TextLabel.Text = "Decompile failed!"
                    return
                end
            end
